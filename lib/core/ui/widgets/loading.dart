...
class _LoadingState extends State<Loading> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animation = Tween(begin: .0, end: 1.0).animate(
        CurvedAnimation(parent: _controller, curve: const Interval(.0, .425)));
    _animation1 = Tween(begin: .0, end: 1.0).animate(
        CurvedAnimation(parent: _controller, curve: const Interval(.4, .85)));
    _rotate = Tween(begin: .0, end: 1.0).animate(CurvedAnimation(
        parent: _controller,
        curve: const Interval(.0, .9, curve: Curves.fastOutSlowIn)));
  }
        ...
        child: AnimatedBuilder(
          ...
          builder: (_, __) => Transform(
            transform: Matrix4.identity()
              ..rotateZ(((_rotate.value) * math.pi * 2) - math.pi / 2),
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ...
      ..shader = ui.Gradient.linear(
          Offset(0, size.height), Offset(size.width, size.height), colors);
